...
import rsa.util.*
	...
	for cMappingVoxI=1:nVox_mappingMask_request
		...
		size(searchlightRDM); %it is 120 despite being a 16x16 because it only the upper triangle

        %run a two-sample paired t-test to test difference between groups if only two conditions 
%         if length((unique(modelRDMs_ltv(~isnan(modelRDMs_ltv))))) == 2
%             
%             [numMod,numSquares] = size(modelRDMs_ltv);
%              
%             entriesModelRDM = (unique(modelRDMs_ltv(~isnan(modelRDMs_ltv)))); 
%             
%             %Fisher's z transform neural RDM scores. The formula to transform r to a z-score is: z? = .5[ln(1+r) ? ln(1-r)]
%             searchlightRDMFish = .5 * (log(1+ searchlightRDM) - log(1-searchlightRDM));
%             
%             for models = 1:numMod
%                 %select those entries in neural RDM where modelRDMs_ltv is value 1
%                 cond1 = searchlightRDMFish(modelRDMs_ltv(models,:)==entriesModelRDM(1));
%                 cond2 = searchlightRDMFish(modelRDMs_ltv(models,:)==entriesModelRDM(2));
%                 [~,p,~,stats] = ttest2(cond2,cond1); %testing that those with more dissim in model RDM have greater dissim values; paired-samples
%                 rs(1,models) = stats.tstat;
%                 ps(1,models) = p;
%             end 
%         else %if more than two conditions, use Spearman's as you normally would
            try
