%% This script is going to read in the neural RDMs for the dependent and independent condition at time of door onset
...
analysisName = 'onsetSearchlight';

%don't make into triangular matrix for Spearmans correlation because the
%neural RDM is not symmetric around the diagonal

load(['/Volumes/Samsung_T5/gems/singleTrialModelNeil/Onset/RDMs',analysisName,'_fMRISearchlight_RDMs.mat']); 
...
for sub = 1:numel(fieldnames(searchlightRDMs))
    ...
    newInfo.fname = fullfile(['/Volumes/Samsung_T5/gems/singleTrialModelNeil/Onset/Interaction_allSubsQuartiles/differenceMap_',analysisName,'_sub',num2str(sub),'.nii']);
    newInfo.descrip = 'tmap';

    %write map, which will have positive values where the difference
    %between diagonal and off-diagonal similarity is greater for dependent
    %than independent blocks and negative values where the opposite is the
    %case
