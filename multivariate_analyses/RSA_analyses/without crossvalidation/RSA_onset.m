function [pDep, statsDep,ciDep, pIndep, statsIndep,ciIndep, pInteraction, statsInteraction, ciInteraction] = RSA_onset(mask, fileName)
...
% TAKES INPUTS: mask and fileName -> save all data
% under what name

%% Step 1: load the masked voxel data

load(['/Users/leonieglitz/Desktop/Garrett&Glitz_etal_2021/onset_fMRI_data/BOLD_onset_',mask,'.mat']);
...
for sub = 1:29
    ...
    %loop through contexts (gems) and find corresponding voxel entries
    for context = 1:4
        for probRange = 1:4
            currentVoxelsGlobal =[];
            for sess = 1:4 %concatenate trial x voxel matrices for the four sessions
                currentVoxelsGlobal = [currentVoxelsGlobal; BOLD(sub).sess(sess).gem(context).prob(probRange).data]; %concatenate across sessions/blocks
            end
            
            meanCurrentVoxels = nanmean(currentVoxelsGlobal,1); %mean across trials
            currentVoxelsReshaped((context-1)*4+probRange,:) = meanCurrentVoxels; %save as 16xvoxels matrix to be able to use pdist to obtain a 16x16 correlation matrix
        end
    end
