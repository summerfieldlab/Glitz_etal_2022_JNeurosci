...
function em(data,subs,X,betas,sigma::Vector,likfun; emtol=1e-4, parallel=false, startx = [], maxiter=200, quiet=false, full=false)
	if full
...
function em(data,subs,X,betas,sigma,likfun; emtol=1e-4, parallel=false, startx = [], maxiter=200, quiet=false, full=false)
	nsub = size(X,1)
    nparam = size(betas,2)
