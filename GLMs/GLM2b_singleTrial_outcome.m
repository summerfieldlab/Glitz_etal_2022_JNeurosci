%% This script runs GLM2b - it creates single-trial outcome timepoint 
%                           events to use in subsequent RSA analyses
...
curr_model_path = '/singleTrialModelNeil/Outcome';
    ...
    for k = 1:blockNo;
        ...
        %=========================================================================
            ...
            %door onset screen - loop over
                ...
                names{1} = 'door_onset';
                onsets{1} = onset.sub(sub_ns(n)).sess(k).categorical.door;
                durations{1} = 0; %stick function
                orth{1} = 0; %off
                

            
            %loop to create different events      
            for j=1:length(onset.sub(sub_ns(n)).sess(k).categorical.outcome)

                    names{j+1} = ['outcome_onset_' num2str(j)];
                    onsets{j+1} = onset.sub(sub_ns(n)).sess(k).categorical.outcome(j);
                    durations{j+1} = 0; %delta (stick) function
                    orth{j+1} = 0; %off
